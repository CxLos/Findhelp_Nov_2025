...
sheet_url = "https://docs.google.com/spreadsheets/d/1GWnQrLptjkgg8CR1G8OpYaCHZMmW5xOzg0kFtPCkxKw/edit?gid=34671814#gid=34671814"
...
df.rename(
    columns={
        'Status': 'Status',
        'Are you a veteran of the US Armed Forces?': 'Veteran',
        'Birthdate': 'Individual\'s Date of Birth:',
        'Do you utilize any of these benefits or social services? Check all that apply:': 'Benefits',
        'Gender': 'Gender',
        'How can BMHC support you today?': 'Support',
        'Race/Ethnicity': 'Ethnicity',
        'What is your living situation?': 'Housing',
        'ZIP Code': 'ZIP Code:',
        'submitted_by' : 'Person',
    },
    inplace=True
...
# Groupby Race/Ethnicity:
df_veteran = df['Veteran'].value_counts().reset_index(name='Count')
...
veteran_bar=px.bar(
    ...
    # width=1000,
    title=dict(
        text='Veteran Distribution Bar Chart',
        x=0.5, 
        font=dict(
            size=25,
            family='Calibri',
...
veteran_pie=px.pie(
    ...
    # height=700, 
    title='Ratio of Veteran Distribution',
    title_x=0.5,
    font=dict(
        family='Calibri',
        size=17,
...
# Groupby Race/Ethnicity:
df_benefits = df['Benefits'].value_counts().reset_index(name='Count')
...
benefits_bar=px.bar(
    ...
    # width=1000,
    title=dict(
        text='Benefits Distribution Bar Chart',
        x=0.5, 
        font=dict(
            size=25,
            family='Calibri',
...
benefits_pie=px.pie(
    ...
    # height=700, 
    title='Benefits Distribution Pie Chart',
    title_x=0.5,
    font=dict(
        family='Calibri',
        size=17,
...
support_unique = [
'MAP', 'Primary care', 'PCP appointment', 'MAP application', 'Mental health', 'Specialty care', 'Mental & PCP appointment', 'MAP enrollment', 'Mental Health Support', 'MAP application & PCP appointment', 'MAP applicTION', 'PCP Appointment', 'MAP Application', 'Dental care', 'PCP Apointment', 'MAP & PCP Appointment', 'PCP appointment & MAP', 'MAP & PCP appointment'
]
...
df['Support'] = (
    df['Support']
        ...
        .replace({
            # MAP variations
            'MAP': 'MAP Appt',
            'MAP application': 'MAP Appt',
            'MAP enrollment': 'MAP Appt',
            'MAP applicTION': 'MAP Appt',
            'MAP Application': 'MAP Appt',
            'MAP application & PCP appointment': 'MAP Appt',
            'MAP & PCP Appointment': 'MAP Appt',
            'MAP & PCP appointment': 'MAP Appt',

            # PCP variations
            'Primary care': 'PCP Appointment',
            'PCP appointment': 'PCP Appointment',
            'PCP Appointment': 'PCP Appointment',
            'PCP Apointment': 'PCP Appointment',
            'PCP appointment & MAP': 'PCP Appointment',

            # Mental Health variations
            'Mental health': 'Mental Health Appt',
            'Mental Health Support': 'Mental Health Appt',
            'Mental & PCP appointment': 'Mental Health Appt',

            # Specialty Care
            'Specialty care': 'Specialty Care',

            # Dental Care
            'Dental care': 'Dental Appt',

            # Other support - these don't have direct matches in your categories
            # You may want to add them or create new categories
        })
)
...
# df_support = pd.DataFrame(counter.items(), columns=['Housing', 'Count']).sort_values(by='Count', ascending=False)
...
housing_pie = px.pie(
    ...
    title='Housing Distribution Pie Chart',
    # height=700, 
    title_x=0.5,
    font=dict(
        family='Calibri',
...
status_pie=px.pie(
    ...
    # height=700, 
    title='Ratio of Eligibility Status',
    title_x=0.5,
    font=dict(
        family='Calibri',
        size=17,
...
person_unique = [
    'jaqueline.oviedo@blackmenshealthclinic.org', 
    'viviana.varela@youthbuildatx.org',
    'michael.lambert@blackmenshealthclinic.org'
]
...
df['Person'] = (
    df['Person']
    .astype(str)
    .apply(lambda x: re.sub(r'@.*', '', x).replace('.', ' ').title())
)
